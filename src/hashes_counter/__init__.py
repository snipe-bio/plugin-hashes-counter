...
# Set the logging level to DEBUG to capture all log levels
...
# Create a formatter and add it to the handler (no timestamp)
...
# Disable propagation to avoid duplicate log entries from the root logger
logger.propagate = False

# Also configure the root logger (used by third-party libraries) to prevent duplicate logs
root_logger = logging.getLogger()
root_logger.setLevel(logging.DEBUG)  # Set root logger level to capture all messages
